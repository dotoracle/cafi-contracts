...
const WEIGHT_MULTIPLIER: u64 = 1000000; // 10^6
...
// this is pool_info
fn get_pool_info(pool_id: usize) -> PoolInfo {
    // let lp_token_key = make_dictionary_item_key_for_key(lp_token);
    // let pool_info_str =
    //     get_dictionary_value_from_key::<String>(POOL_INFO, &lp_token_key).unwrap_or_revert();
    ...
    let target_pool_id = list[pool_id].clone();
    target_pool_id
}
...
pub extern "C" fn un_stake() -> Result<(), Error> {
    ...
    transfer_from_erc20_token(pool_info.lp_token.clone(), this_key, caller, amount);
    user_info_of_this_pool.reward_debt = user_info_of_this_pool.total_stake_amount
        * pool_info.acc_reward_per_share
        / U256::pow(U256::from("10"), U256::from("18"));
...
fn pay_rewards(payer: Key, user: Key, rewards_amount: U256, mut user_info_of_this_pool: UserInfo) -> UserInfo {
    // transfer rewards
    ...
    let reward_token_balance_of_contract: U256 = get_balance_erc20_token(rewards_token, user);
    if (reward_token_balance_of_contract) < rewards_amount {
        transfer_from_erc20_token(rewards_token, payer, user, reward_token_balance_of_contract);
        user_info_of_this_pool.pending_rewards =
            user_info_of_this_pool.pending_rewards - reward_token_balance_of_contract;
    } else {
        transfer_from_erc20_token(rewards_token, payer, user, rewards_amount);
        user_info_of_this_pool.pending_rewards = U256::from(0);
    }
...
fn update_pool(pool_id: usize) {
    ...
    let mut this_pool = &mut list[pool_id];
    // let mut this_pool: PoolInfo = get_pool_info(pool_id);
    ...
    // Todo: update pool
    let multiplier: U256 = get_multiplier(
        this_pool.last_reward_second.clone(),
        current_block_timestamps,
    );
    let total_alloc_point: U256 = helpers::get_stored_value_with_user_errors::<U256>(
        TOTAL_ALLOC_POINT,
        Error::MissingTotalAllocPoint,
    ...
    let reward: U256 =
        multiplier * reward_per_second * this_pool.alloc_point.clone() / total_alloc_point;
    // Todo: transfer reward to this contract

    //update
    this_pool.acc_reward_per_share = this_pool.acc_reward_per_share
        + (reward * U256::pow(U256::from("10"), U256::from("18")) / this_pool.lp_supply);
...
#[no_mangle]
pub extern "C" fn get_pending_rewards() -> Result<(), Error> {
    // This is equal to pool_id
    let lp_token: Key = helpers::get_named_arg_with_user_errors(
        ARG_LP_CONTRACT_HASH,
...
fn transfer_from_erc20_token(token: Key, source: Key, target: Key, amount: U256) {
...
fn get_balance_erc20_token(token: Key, owner: Key) -> U256 {
    ...
    let balances: U256 = runtime::call_contract(
        contract_hash,
